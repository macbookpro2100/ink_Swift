...
class HeroMainViewController: UIViewController {
  ...
  func setupcollection() {
    ...
    let footimageView = UIImageView(image: #imageLiteral(resourceName: "macbookpro_portrait_small_2x"))
    footimageView.contentMode = .scaleAspectFit
    
    let footimageProvider = SimpleViewProvider(views: [footimageView], sizeStrategy: (.fill, .fit))
    ...
    collectionView.provider = ComposedProvider(
      ...
      sections: [imageProvider, examplesProvider, footimageProvider]
    )
  }
