...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _displayLabel.backgroundColor = [UIColor colorFromHexString:@"#AAAAAA"];
...
- (void)showText:(NSString *)text{
    _displayLabel.frame = CGRectMake(self.width/2-150/2, self.height/2-100/2, 150, 100);
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.25 animations:^{
        __strong __typeof(weakSelf) strongSelf = weakSelf;
        strongSelf.displayLabel.frame = CGRectMake(self.width/2-300/2, self.height/2-200/2, 300, 200);
