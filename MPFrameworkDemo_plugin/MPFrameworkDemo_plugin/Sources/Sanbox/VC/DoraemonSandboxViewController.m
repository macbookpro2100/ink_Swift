...
- (void)loadPath:(NSString *)filePath {
    ...
    if (!targetPath || [targetPath isEqualToString:_rootPath]) {
        ...
        self.title = dirTitle;
        self.navigationItem.leftBarButtonItem = [AUBarButtonItem barButtonItemWithImageType:AUBarButtonImageTypeBack target:self action:@selector(leftNavBackClick:)];
    }
    model.path = filePath;
