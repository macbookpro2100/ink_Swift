//
//  InkFormTableViewController.swift
//  ink_Swift
//
//  Created by 李砚 on 2017/7/30.
...
class InkFormTableViewController: UITableViewController {

    fileprivate let itemDataSouce: [[(name: String, viewControllerName: FormViewController)]] = [
        [
            ("Rows", RowsExampleViewController.init(style: .grouped)),
             ("Native iOS Event Form", NativeEventFormViewController.init()),
        
            ("Accesory View Navigation", NavigationAccessoryController.init()),
            ("Custom Cells", CustomCellsController.init(style: .grouped)),
//            ("Customization of rows with text input",FieldRowCustomizationController.init(style: .grouped)),
            ("Hidden rows", HiddenRowsExample.init(style: .grouped)),
             ("Disabled rows", DisabledRowsExample.init(style: .grouped)),
              ("Formatters", FormatterExample.init(style: .grouped)),
               ("Inline rows", InlineRowsController.init(style: .grouped)),
                ("List Sections", ListSectionsController.init(style: .grouped)),
                 ("Validations", ValidationsController.init(style: .grouped)),
                 ("Custom Design", CustomDesignController.init(style: .grouped)),
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "InkFormTableViewController"
   ...
   override  func numberOfSections(in tableView: UITableView) -> Int {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let item = self.itemDataSouce[indexPath.section][indexPath.row]
        let className: FormViewController  = item.viewControllerName
        print("点击了\(className)")
