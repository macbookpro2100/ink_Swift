...
//  Ink_Swift
//
//  Created by LXFWeChat on 2016/12/29.
...
//        let avatarModel = LXFSettingCellModel(icon: #imageLiteral(resourceName: "SettingProfileHead"), title: Ink_SwiftTools.shared.getMineInfo()?.userInfo?.nickName ?? "用户名",  tipImg: nil, tipTitle: nil, type: .avatar)
//        avatarModel.subTitle = Ink_SwiftTools.shared.getMineInfo()?.userId
        return [
//            [avatarModel],
            [
    ...
    class func getProfileData() -> [[LXFSettingCellModel]] {
        ...
        let userId = "未设置"
        let tipImgStr =  "SettingProfileHead"
        
        return [
            [
                LXFSettingCellModel(icon: nil, title: "头像", tipImg: tipImgStr, tipTitle: nil, type: .avatar),
                LXFSettingCellModel(icon: nil, title: "名字", tipImg: nil, tipTitle: "未设置", type: .default),
