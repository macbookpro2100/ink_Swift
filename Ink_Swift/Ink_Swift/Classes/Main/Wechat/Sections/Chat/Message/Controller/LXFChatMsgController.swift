...
//  Ink_Swift
//
//  Created by LXFWeChat on 2017/1/3.
...
class LXFChatMsgController: LXFBaseController {
    ...
    lazy var dataArr: [LXFChatMsgModel] = {
        let history = Ink_SwiftTools.shared.getLocalMsgs(userId: self.user?.userId)
...
extension LXFChatMsgController {
    ...
    fileprivate func setup() {
        ...
        // 设置当前聊天的userId
        Ink_SwiftTools.shared.currentChatUserId = user?.userId
...
extension LXFChatMsgController {
    func loadEarlierMsgs() {
        LXFLog("加载更早的消息")
        let earlierMsgs = Ink_SwiftTools.shared.getLocalMsgs(userId: user?.userId, message: dataArr.first?.message)
        ...
        if eMsgs.count == 0 {
            ...
            // 加载云端消息
            Ink_SwiftTools.shared.getCloudMsgs(currentMessage: curMessage, userId: user?.userId, resultBlock: { (error, messages) in
                if error != nil {
                    LXFLog(error)
