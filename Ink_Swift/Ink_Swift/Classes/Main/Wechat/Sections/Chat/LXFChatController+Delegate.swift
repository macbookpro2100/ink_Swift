...
// MARK:- Ink_SwiftToolsMediaDelegate
extension LXFChatController : Ink_SwiftToolsMediaDelegate {
    /* ============================== 播放音频的回调 ============================== */
    // 准备开始播放音频
    ...
    func weChatToolsMediaRecordAudio(_ filePath: String, didCompletedWithError error: Error?) {
        ...
        if error != nil {
            ...
            // 发送音频
            Ink_SwiftTools.shared.sendMedia(userId: user?.userId, filePath: filePath, type: .audio)
        }
    }
    // 获得当前录音时长
    func weChatToolsMediaRecordAudioProgress(_ currentTime: TimeInterval) {
        let averagePower = Ink_SwiftTools.shared.getRecordVoiceAveragePower()
