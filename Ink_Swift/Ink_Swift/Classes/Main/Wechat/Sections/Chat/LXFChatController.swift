...
//  Ink_Swift
//
//  Created by LXFWeChat on 2016/12/30.
...
extension LXFChatController {
    fileprivate func setup() {
        ...
        // 注册通知
        self.registerNote()
        Ink_SwiftTools.shared.mediaDelegate = self
    }
    // 注册通知
...
extension LXFChatController {
    ...
    func chatBarRecordBtnLongTapBegan(_ note : Notification) {
        ...
        // 开始录音
        Ink_SwiftTools.shared.recordVoice()
    }
    ...
    func chatBarRecordBtnLongTapEnded(_ note : Notification) {
        ...
        if finishRecordingVoice {
            // 停止录音
            Ink_SwiftTools.shared.stopRecordVoice()
        } else {
            // 取消录音
            Ink_SwiftTools.shared.cancelRecordVoice()
        }
