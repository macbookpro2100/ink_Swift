//
//  Ink_SwiftViewController+Notification.swift
//  Ink_Swift
//
//  Created by LXFWeChat on 2017/7/24.
...
extension Ink_SwiftViewController {
    @objc fileprivate func receiveMsg(_ note: Notification) {
        ...
        // 收到自己发送信息的通知则不用理会
        if nimMsg.from == Ink_SwiftTools.shared.getMineInfo()?.userId { return }
...
//            LXFLog("是自定义消息 \(type)")
            // 这里暂时就只有视频聊天
            let user = LXFContactCellModel(user: Ink_SwiftTools.shared.getFriendInfo(userId: nimMsg.from ?? ""))
