...
//  Ink_Swift
//
//  Created by LXFWeChat on 2017/1/30.
...
extension LXFGenderViewController {
    fileprivate func setup() {
        ...
        // 设置cell的数据
//        let mineInfo = Ink_SwiftTools.shared.getMineInfo()
...
extension LXFGenderViewController {
    @objc fileprivate func goBack(_ note: Notification) {
        guard let type = note.object as? Ink_SwiftToolsUpdateType else {
            return
        }
        if type == .avatar {
...
extension LXFGenderViewController {
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        var gender: Ink_SwiftToolsGender!
...
//        Ink_SwiftTools.shared.updateGender(with: gender)
    }
}
